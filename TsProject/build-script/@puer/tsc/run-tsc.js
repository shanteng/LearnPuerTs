...
export default function (tsConfigPath) {
    ...
    const cl = ts.getParsedCommandLineOfConfigFile(tsConfigPath, {}, Object.assign({ onUnRecoverableConfigFileDiagnostic: (d) => d }, ts.sys));
    if (!cl) {
...
export function watch(tsConfigPath, onFileChanged = (name, text) => { }) {
    const cl = ts.getParsedCommandLineOfConfigFile(tsConfigPath, {}, Object.assign({ onUnRecoverableConfigFileDiagnostic: (d) => d }, ts.sys));
    if (!cl) {
        throw new Error('parse tsconfig failed: ' + tsConfigPath);
    }
    const files = {};
    const rootFileNames = cl.fileNames.reduce((prev, cur) => {
